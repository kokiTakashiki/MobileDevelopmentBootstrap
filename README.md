...
- **`make setup`**: 新 Mac を 1 コマンドで開発可能な状態まで持っていく起動装置
  ...
  - Brewfile による GUI / CLI アプリの一括導入（Xcode / Android Studio / VSCode / IntelliJ / Nix / direnv 等）
  - 手作業項目（Apple ID サインイン・証明書・Android SDK 等）の対話的ガイド
...
git clone https://github.com/kokiTakashiki/macOSBootstrap.git ~/Developer
...
| `setup-manual` | Apple ID / 証明書 / Android SDK / VSCode 拡張の案内 | 手動（対話停止） |
...
- **Mac App Store CLI (`mas`)** — Xcode の自動インストール
...
- **Xcode は Brewfile + mas 経由**: Apple EULA 上 Nix での再配布が不可能なため、`mas` 経由で Mac App Store からインストール。
...
- **Xcode のバージョン管理が困難**: Brewfile では 1 バージョンしか管理できない。複数バージョンが必要な場合は [xcodes-cli](https://github.com/XcodesOrg/xcodes) の導入を検討。

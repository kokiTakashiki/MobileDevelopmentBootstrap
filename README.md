...
- **`make setup`**: 新 Mac を 1 コマンドで開発可能な状態まで持っていく起動装置
  ...
  - Brewfile による GUI / CLI アプリの一括導入（Android Studio / JetBrains Toolbox / VSCode / Nix / direnv / xcodes / git / gh）
  - 手作業項目（`xcodes` による Xcode インストール・証明書・Toolbox 経由の IntelliJ 等・Android SDK・VSCode 拡張）の対話的ガイド
...
| `setup-manual` | xcodes による Xcode インストール / 証明書 / Toolbox 経由の IntelliJ 等（任意）/ Android SDK / VSCode 拡張の案内 | 手動（対話停止） |
